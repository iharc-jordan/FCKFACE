...
Additional source-only review found an explicit alignment path in TransFace's pinned [IJB-C evaluator](https://github.com/DanJun6737/TransFace/blob/fb6cd56d04c7ea525e328219f5d5066adac03ada/eval_ijbc.py): five-point similarity alignment to the 112-pixel InsightFace template, OpenCV linear warp with zero border, then RGB and `[-1,1]` normalization. That evaluation uses original and horizontally flipped crops and sums their features before template aggregation. Its landmarks are supplied by dataset metadata; it does not resolve the detector for arbitrary photographs. The simple `inference.py` example instead calls `.numpy()` on the model's multi-output result, whereas the evaluator explicitly unpacks the feature tensor. A future adapter must verify the actual output signature and predeclare whether it uses flip augmentation. These code observations do not select a new checkpoint, load the reserved model, or establish parity with a complete author pipeline.
