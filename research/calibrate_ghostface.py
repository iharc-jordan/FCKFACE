"""Calibrate development GhostFaceNet on FRLL calibration identities only.
...
from fckface_lab.ghostface import GhostFaceModel, author_align
...
def source_hashes() -> dict[str, str]:
    ...
    files = ("calibrate_ghostface.py", "fckface_lab/ghostface.py",
             "fckface_lab/calibration.py",
...
def verify_models(yunet_path: Path, ghostface_path: Path) -> dict[str, dict]:
    registry = json.loads((Path(__file__).resolve().parent / "models.json").read_text())
    results = {}
    for name, path in (("yunet", yunet_path), ("ghostface", ghostface_path)):
        entry = registry["development"].get(name) if name == "yunet" else {
            "filename": "ghostfacenet_v1.h5",
            "sha256": "e4182ca2470dac3eb79974f4d5d54f2abdf66d7e231c6f3df4059a90bded1271",
            "bytes": 17271840,
            "source": "https://github.com/HamadYA/GhostFaceNets/releases/tag/v1.2"}
...
def main() -> int:
    ...
    parser.add_argument("--ghostface", type=Path, required=True)
    ...
    models = verify_models(args.yunet, args.ghostface)
    cv2.setNumThreads(2)
    started = utc_now()
    ...
    try:
        recognizer = GhostFaceModel(args.yunet, args.ghostface)
        parity = recognizer.selftest()
        add_log(log_path, {"event": "author_h5_synthetic_parity", "at_utc": utc_now(), **parity})
            ...
            try:
                if digest(path) != row["sha256"]:
                    ...
                    if (status == "valid" and row["view"] in diagnostic_views
                        ...
                        crop = author_align(image, embedded.landmarks)
                        crop_path = diagnostic_dir / f"{row['view']}-aligned.png"
                        Image.fromarray(crop).save(crop_path)
                        diagnostic_images[-1]["aligned_relative_path"] = crop_path.relative_to(output).as_posix()
                        diagnostic_images[-1]["aligned_sha256"] = digest(crop_path)
        ...
        artifact = {
            "schema_version": 1, "purpose": "development_author_h5_ghostfacenet_calibration_only",
            ...
            "dataset": {"name": "frll", "split": "calibration",
                        ...
                        "views": list(VIEWS)},
            "pipeline": {"detector": "OpenCV FaceDetectorYN YuNet (author detector substitution)",
                         "recognizer": "Serialized author GhostFaceNetV1 H5 via legacy TensorFlow Keras, mixed_float16 graph as saved",
                         "face_selection": "exactly_one_detected_face",
                         "alignment": "skimage SimilarityTransform five-point YuNet RGB112 inverse warp times255 uint8 truncation",
                         "embedding": "float32 (RGB-127.5)/128 fed to saved graph; L2-normalized 512D output",
                         ...
                         "detector_input": {
                             ...
                             "recognizer_input": "author-aligned RGB112 uint8 crop from original full-resolution decoded image",
                         },
                         "cv2_threads": cv2.getNumThreads(),
                         "source_sha256": source_hashes(), "model_artifacts": models,
                         "synthetic_parity": parity,
            ...
            "threshold_uncertainty": {
                ...
                "maximum": max(item["threshold"] for item in loo_thresholds),
                           "caveat": "Pair scores share only 20 identities; empirical FMR and leave-one-identity-out range are not population confidence bounds. Detection failures reduce valid coverage. YuNet substitutes for the author detector. This is development GhostFaceNet, not held-out validation."},
